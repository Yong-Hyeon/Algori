...
public class Main {
    public static void main(String[] args) throws Exception{
        ...
        for(int i = 0; i < M + K; i++){
            ...
            if(a == 1){
                update(input, segmentTree, 1, 1, N, b, c, input[(int)b] - c);
            }else{
    ...
    private static long search(long[] segmentTree, int node, int left, int right, int start, int end) {
        ...
        return search(segmentTree, node * 2, left, mid, start, end) + search(segmentTree, node * 2 + 1, mid + 1, right, start, end);
    }

    private static void update(long[] input, long[] segmentTree, int node, int left, int right, long target, long change, long diff) {
        ...
        int mid = (left + right) / 2;
        update(input, segmentTree, node * 2, left, mid, target, change, diff);
        update(input, segmentTree, node * 2 + 1, mid + 1, right, target, change, diff);
    }
